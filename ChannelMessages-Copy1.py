...
#FUNCTION TO READ TELEGRAM MESSAGE WITH NO EMOJI
...
##############################################################################
#NOW START THE MAIN PART OF PROGRAM 
...
async def main(phone):
    ...
    ##########################################################################################
    #GET JUST THE LATEST MESSAGE FROM TELEGRAM
    while True:
    ...
    #thêm 1 đoạn để regex từ message thành 1 dict chứa thông tin lệnh
    latest_message_text = latest_message['message']
    
                          
    my_trade = {}
    my_trade['_action'] = 'OPEN'
    my_trade['_type'] = orderTypeEncode(latest_message_text.split()[1])
    my_trade['_symbol'] = latest_message_text.split()[0]
    my_trade['_price'] = 0.0
    my_trade['_SL'] = 0.0
    my_trade['_TP'] = 0.0
    my_trade['_lots'] = 0.01
    ...
    reply = _zmq._get_response_()
        

    with open('reply.json', 'w') as outfile:
        json.dump(reply, outfile, cls=DateTimeEncoder)
